...
function findZeroSumArrays(arr: number[]): number[][] {
    const result: number[][] = [];
    for (let i = 0; i < arr.length - 2; i++) {
      for (let j = i + 1; j < arr.length - 1; j++) {
        for (let k = j + 1; k < arr.length; k++) {
          if (arr[i] + arr[j] + arr[k] === 0) {
            const tempArray: number[] = [arr[i], arr[j], arr[k]];
  ...
  function checkUnique(array: number[], result: number[][]): boolean {
  ...
  const a: number[] = [1, 2, -1, 0, -1,9,-8,-1];
  const zeroSumArrays: number[][] = findZeroSumArrays(a);
