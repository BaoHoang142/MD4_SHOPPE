class Student {
    ...
    constructor(studentCode: number, studentName: string, age: number, address: string, phoneNumber: number) {
...
function updateStudent(i: number, student: Student) {
    students[i] = student;
    
}
function getUserByCurrentStudent(i: number) {
    let index = 0;
    let userCheck;
    ...
    switch (option) {
        ...
        case "u":
            ...
            if (!studentUpdate.userCK) {
                ...
                let update = Number(
                  prompt( "nhập 1 để thay đổi tên,2 để thay đổi tuổi,3 để thay đổi địa chỉ,4 để thay đổi sđt,5 để thoát"
                  )
                );
                switch (update) {
                  case 1:
