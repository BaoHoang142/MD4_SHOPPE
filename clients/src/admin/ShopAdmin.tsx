...
const ShopAdmin = () => {
  ...
  const getStores = async () => {
    try {
      let response = await privateAxios.get("/api/v1/stores/admin/user");
  ...
  const handleLock = async (id: string) => {
    ...
    try {
      let response = await axios.put(
        `http://localhost:8080/api/v1/stores/status/${id}`,
        {
          statusStore,
        }
      );
  ...
  const handleSearch = async () => {
    try {
      let storeSearch = await axios.get(
        `http://localhost:8080/api/v1/stores/search/stores?key=${search}`
      );
